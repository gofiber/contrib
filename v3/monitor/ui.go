...
var dashboardTemplate = template.Must(template.New("monitor").Parse(dashboardHTML))
...
func renderDashboard(config Config) (string, error) {
	...
	if config.FaviconURL == "" {
		faviconURL = template.URL(defaultMonitorFaviconURL) // #nosec G203 -- package-owned embedded SVG.
	}
	descriptorLabel := "FDs"
