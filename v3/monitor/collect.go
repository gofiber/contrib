...
// Keep the indexed sample table and its names in lockstep. A length mismatch
// fails at compile time instead of silently leaving a runtime metric unnamed.
var _ [runtimeMetricCount]struct{} = [len(runtimeMetricNames)]struct{}{}
...
func (c *collector) collectSystem(now time.Time) (systemStats, []string) {
	...
	// Monitor has no background collectors, so load averages are unsupported there
