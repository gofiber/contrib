...
func TestDashboardContract(t *testing.T) {
	...
	assert.Contains(t, pageHTML, `const metricsPerPage = 5;`)
	assert.Equal(t, 2, strings.Count(pageHTML, `<dt>TCP Connections</dt>`))
	assert.Contains(t, pageHTML, `id="pid-tcp-connections"`)
	assert.Contains(t, pageHTML, `id="os-tcp-connections"`)
...
func TestDashboardInteractions(t *testing.T) {
	...
	for _, series := range []string{"p50", "p95", "p99", "error4xx", "error5xx"} {
		assert.Contains(t, pageHTML, `data: visibleSamples(history.`+series+`)`)
	}
	assert.NotContains(t, pageHTML, `fillForward`)
	assert.NotContains(t, pageHTML, `fillZero`)
	assert.NotContains(t, pageHTML, `rawData`)
	assert.NotContains(t, pageHTML, `dash: [6, 4]`)
	assert.Contains(t, pageHTML, `function niceScale(`)
	assert.Contains(t, pageHTML, `minimumStep: 1`)
	assert.Contains(t, pageHTML, `if (!candidates.length) candidates.push(`)
	assert.Contains(t, pageHTML, `function drawTimeLabels(`)
	assert.Contains(t, pageHTML, `const x = sampleX(labelMeta, entry.index)`)
	assert.Contains(t, pageHTML, `function drawHoverGuides(`)
	assert.Contains(t, pageHTML, `canvas.addEventListener("pointermove"`)
	...
	assert.Contains(t, pageHTML, `name.textContent = point.name`)
	assert.Contains(t, pageHTML, `value.textContent = point.value`)
	assert.Contains(t, pageHTML, `No data in this window`)
	assert.NotContains(t, pageHTML, `No requests in this window`)
