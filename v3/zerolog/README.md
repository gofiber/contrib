...
| RedactHeader    | `func(string, fiber.Ctx) bool`      | Redact a header value when it returns true. Common credential-bearing headers such as `Authorization`, `Cookie`, `Set-Cookie`, `Location`, and CSRF-token headers are redacted by default.                                                           | built-in sensitive-header filter                                         |
